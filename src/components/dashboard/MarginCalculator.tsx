...
export default function MarginCalculator({ totalCost, deliverableGrams }: MarginCalculatorProps) {
  const state = useProjectState();
  const dispatch = useProjectDispatch();
  const margin = calculateMargin(totalCost, state.sellingPricePerGram, deliverableGrams, state.batchCount);
      ...
      <div className="flex items-end gap-4">
        <div className="flex-1">
          ...
          <input
            ...
            placeholder="0.00"
            onChange={(e) =>
              dispatch({ type: 'SET_SELLING_PRICE', payload: Math.max(0, parseFloat(e.target.value) || 0) })
            }
          />
        </div>
        {state.sellingPricePerGram > 0 && (
          <>
            ...
            <div className="text-center">
              ...
              <p className={`text-sm font-semibold ${margin.grossProfit >= 0 ? 'text-success' : 'text-danger'}`}>
                {formatCurrency(margin.grossProfit)}
              </p>
            </div>
            <div className="text-center">
              <p className="text-xs text-gray-500 dark:text-gray-400">Margin</p>
              <p className={`text-sm font-bold ${margin.marginPercent >= 0 ? 'text-success' : 'text-danger'}`}>
